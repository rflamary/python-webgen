...
class LaTeXPreprocessor(markdown.preprocessors.Preprocessor):
    ...
    def __init__(self, configs):
        try:
            cache_file = open('latex2.cache', 'r+')
    ...
    def _latex_to_base64(self, tex, math_mode):
        ...
        status = call(('pdflatex -halt-on-error %s' % path).split(), stdout=PIPE)
    ...
    def run(self, lines):
        ...
        for reg, math_mode, expr in tex_expr:
            ...
            page = reg.sub(data, page, 1)

        # Cache our data
        cache_file = open('latex2.cache', 'a')
